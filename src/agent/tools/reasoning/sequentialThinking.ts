...
function suggestNextSteps(thought: string, availableTools: string[]): SequentialThinkingResult["nextStep"] {
  ...
  const patterns: Array<{
    ...
    {
      tool: "diagnose_scene",
      reason: "Root-cause timing-budget analysis: tells you which element is clipped, the exact frame where it ends, and the fix recipe.",
      args: {},
      triggers: [
        "doesn't appear",
        "doesn't show",
    ...
    {
      tool: "audit_scene",
      reason: "Root-cause layout-logic analysis: tells you which element is misaligned, overflowing, or stacked wrongly, with a concrete update_element patch.",
      args: {},
      triggers: [
        "text isn't on",
        "not on the button",
